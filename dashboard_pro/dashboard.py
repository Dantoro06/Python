...
def _validar_reporte(data: Any) -> Optional[Dict[str, Any]]:
    if not isinstance(data, dict):
        ...
        return None
    faltantes = [k for k in ("metadata", "resumen_ejecucion") if k not in data]
    if faltantes:
        st.error(f"El reporte carece de secciones requeridas: {', '.join(faltantes)}.")
        return None
    return data
...
def _fuente_reporte(uploaded_file) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        st.error(f"No se pudo procesar el archivo subido: {exc}")
    return None
...
eventos_df = _tabla_top(
    reporte.get("top_eventos_riesgo", []) or reporte.get("top_eventos_sospechosos", []),
    {
        "evento_id": "Evento",
