...
def _get_col(df: pd.DataFrame, posibles: List[str]) -> str:
    ...
    de las opciones dadas (ignorando mayúsculas/minúsculas y espacios).
    """
    ...
    # intento por contiene (aquí realmente comparamos igual por seguridad)
    for nombre in posibles:
        clave = nombre.lower().strip()
        for c in df.columns:
            if clave == c.lower().strip():
                return c
...
def preparar_tabla_base_hidding_bonus(df_apuestas: pd.DataFrame) -> pd.DataFrame:
    ...
    col_user = _get_col(df, ["Player Id", "playerid", "userid"])
    col_sport = _get_col(df, ["Sport", "Deporte"])
    col_bet_type = _get_col(df, ["Bet type", "bettype"])
...
def actualizar_historico_riesgo(
    ...
    if os.path.exists(ruta_historico):
        try:
            historico = json.load(open(ruta_historico, encoding="utf-8"))
        except:
            historico={"ejecuciones":[]}
    else:
        historico={"ejecuciones":[]}
    historico.setdefault("ejecuciones", []).append(entry)
    json.dump(historico, open(ruta_historico,"w",encoding="utf-8"), ensure_ascii=False, indent=2)
